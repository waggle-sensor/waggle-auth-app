        ...
        for manifest in manifests:
            ...
            for lc in manifest["lorawanconnections"]:
                lc_obj = LorawanConnection.objects.create(
                    ...
                    lorawan_device=LorawanDevice.objects.get(
                        deveui=lc["lorawandevice"]["deveui"]
                    ),
                )
...
class NodesViewSetTestCase(TestCase):
    ...
    def setUp(self):
        ...
        createSensorHardware(
            [
                {"hardware": "bme280", "hw_model": "BME280", "manufacturer": "Bosch"},
        ...
        createProject([{"name": "Sage"}, {"name": "MyProject"}])

        createFocus([{"name": "URBAN"}, {"name": "MyFocus"}])

        createPartner([{"name": "NU"}, {"name": "MyPartner"}])
    ...
    def test_retrieve_nodes(self):
        ...
        self.assertEqual(
            data[0]["vsn"], self.W021_data["vsn"]
        )  # ordered by vsn so W021 is index 0
    ...
    def test_project_url_mult_filtering(self):
        ...
        self.assertEqual(
            len(data), 2
        )  # 2 since one node in MyProject and other in Sage
        self.assertEqual(
            data[1]["vsn"], self.W123_data["vsn"]
        )  # ordered by vsn so W123 is index 1
    ...
    def test_mix_url_filtering(self):
        ...
        r = self.client.get(
            "/api/v-beta/nodes/?project__name=MyProject,Sage&phase=Maintenance"
        )
        data = r.json()
        self.assertTrue(isinstance(data, list))
